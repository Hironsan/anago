    ...
    def __init__(self, config, embeddings=None, ntags=None):
        ...
        if embeddings is None:
            ...
            word_embeddings = Embedding(
                ...
                mask_zero=True,
                weights=[embeddings],
                trainable=config.train_embeddings)(word_ids)
        ...
        fwd_state = LSTM(
            config.num_char_lstm_units, return_state=True)(char_embeddings)[-2]
        bwd_state = LSTM(
            config.num_char_lstm_units, return_state=True,
        ...
        self.model = Model(
            inputs=[word_ids, char_ids, sequence_lengths], outputs=[pred])
        self.config = config
